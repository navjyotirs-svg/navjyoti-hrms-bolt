...
### Remaining items
1. **2 edge functions need deployment**: daily-report-action, export-handler — source is updated on disk but deploy budget was reached
2. **Calendar/announcement/security notifications**: Event definitions exist in the catalogue but are not yet wired into edge functions
3. **notification_deliveries rows**: `send-push-notification` now creates delivery records for real notifications, but `notification-worker` still has nothing to process for in-app/email channels
...
Replaced `crypto.subtle.importKey("raw", ...)` with a manually-constructed PKCS#8 DER wrapper imported via `importKey("pkcs8", ...)`. This fixed the VAPID JWT signing but push delivery still failed because **payload encryption was never implemented**.

### Round 2: Web Push payload encryption fix (ACTUAL root cause)
The previous custom implementation sent **plain JSON** directly to FCM's endpoint. Web Push requires RFC 8291 payload encryption using the subscriber's `p256dh` and `auth` keys. Without encryption, FCM rejects the request with a 400/404 status, which was masked as "temporary_failure" by the broad catch block.

**Fix**: Replaced all custom VAPID JWT + crypto code with the maintained `web-push` npm library (v3.6.7) imported via `https://esm.sh/web-push@3.6.7`. This library handles:
- VAPID JWT generation with correct ES256 signing
- RFC 8291 payload encryption (aes128gcm content encoding)
- Correct Authorization and Crypto-Key headers
- Proper error categorization from provider responses

### Exact failing layer (final diagnosis)
1. **Primary**: No RFC 8291 payload encryption — plain JSON sent to FCM endpoint, rejected by provider
2. **Secondary**: VAPID JWT signing failed due to `importKey("raw", ...)` for private ECDSA key (requires PKCS#8)
3. **Tertiary**: Broad `catch` block masked all errors as `temporary_failure`

### VAPID validation result
- VAPID_PUBLIC_KEY: found, valid (65 bytes uncompressed P-256 point)
- VAPID_PRIVATE_KEY: found, valid (32 bytes scalar)
- VAPID_SUBJECT: found, starts with `mailto:`
- Key pair validated by `web-push` library's `setVapidDetails()`
- Private key never in frontend bundle

### Frontend function called
- Function: `send-test-push` (canonical, no duplicates)
- URL: `${VITE_SUPABASE_URL}/functions/v1/send-test-push`
- Method: POST
- The `send-push-notification` function also has a `test=true` path but is NOT called by the frontend for test pushes
- Frontend `sendTestPushNotification()` calls `send-test-push` only

### Structured response returned to UI
```json
{
  "functionVersion": "v4-webpush-lib",
  "correlationId": "<uuid>",
...
### Safe error categories returned
- `AUTHENTICATION_FAILED` — no valid session
- `NO_ACTIVE_SUBSCRIPTION` — no active push subscriptions
- `VAPID_SECRET_MISSING` — VAPID keys not configured
- `VAPID_KEY_INVALID` — VAPID key format mismatch or stale fingerprint
- `SUBSCRIPTION_EXPIRED` — provider returned 404/410
- `PUSH_PROVIDER_UNAUTHORIZED` — provider returned 401/403
- `PUSH_PROVIDER_BAD_REQUEST` — provider returned 400
- `PUSH_PROVIDER_RATE_LIMITED` — provider returned 429
- `PUSH_PROVIDER_ERROR` — provider returned 5xx
- `NETWORK_TIMEOUT` — fetch timeout/network error
- `UNKNOWN_SERVER_ERROR` — unhandled exception

### Database subscription verification (2026-07-25)
- 2 active subscriptions (one per user)
- Both have HTTPS endpoints (FCM)
- Both have non-empty p256dh_key and auth_key
- Both have `permission_status = 'granted'`
- Both have `vapid_key_fp = 'BFisCU4FXyYAR_el'` (matches current VAPID public key)
- Both have valid `organization_id`

### Files changed (final)
- `supabase/functions/send-test-push/index.ts` — rewritten with `web-push` library, structured response, safe error categories, correlationId, functionVersion
- `supabase/functions/send-push-notification/index.ts` — rewritten with `web-push` library, same structured response
- `supabase/functions/subscribe-device/index.ts` — duplicate cleanup on replace (unchanged from round 1)
- `src/lib/webPush.ts` — `sendTestPushNotification` displays structured error categories, `repairPushSubscription` cleans duplicates, `saveSubscriptionToServer` accepts `replace` param
- `public/sw.js` — versioning, SKIP_WAITING message handler (unchanged from round 1)
- `src/lib/__tests__/push.test.ts` — 41 tests total, updated for web-push library assertions

### Functions deployed (round 2)
- send-test-push: DEPLOYED (v4-webpush-lib)
- send-push-notification: DEPLOYED (v4-webpush-lib)
- subscribe-device: DEPLOYED

### Functions still needing deployment
- daily-report-action: source updated, NOT YET DEPLOYED
- export-handler: source updated, NOT YET DEPLOYED
...
### Manual production verification
NOT YET PERFORMED — requires browser testing on https://hrms.ngspl.com/settings. The user should:
1. Open Account Settings on https://hrms.ngspl.com/settings
2. Click "Repair Push Subscription" — creates one fresh subscription with current VAPID key
