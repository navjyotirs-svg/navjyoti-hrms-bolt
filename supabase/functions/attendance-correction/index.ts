...
async function handleRequestCorrection(
  ...
  await notifyBusinessEvent(admin, {
    eventCode: "ATTENDANCE_CORRECTION_SUBMITTED",
...
async function handleReviewCorrection(
  ...
  if (body.decision === "APPROVED") {
    ...
    if (newCheckOut) {
      ...
      const requiredTotal = record.required_total_minutes ?? 540;
      newStatus = newElapsed >= requiredTotal ? "FULL_DAY" : "HALF_DAY";
      statusReason = `Corrected: ${newElapsed} minutes elapsed (full-day threshold: ${requiredTotal})`;
    }
    ...
    if (correction.requested_check_in_at) {
      newRequiredCheckout = new Date(
        new Date(newCheckIn).getTime() + (record.required_total_minutes ?? 540) * 60 * 1000
      ).toISOString();
    }
  ...
  // Supervisory notification: notify the employee (includeEmployee: true)
  await notifyBusinessEvent(admin, {
    eventCode: body.decision === "APPROVED" ? "ATTENDANCE_CORRECTION_APPROVED" : "ATTENDANCE_CORRECTION_REJECTED",
    actorUserId: callerId,
    employeeId: correction.employee_id,
    organizationId: profile.organization_id,
    entityType: "attendance_correction",
    entityId: correction.id,
    title: body.decision === "APPROVED" ? "Attendance Correction Approved" : "Attendance Correction Rejected",
    message: body.decision === "APPROVED"
      ? "An attendance correction request has been approved."
      : "An attendance correction request has been rejected.",
    priority: "normal",
    category: "attendance",
